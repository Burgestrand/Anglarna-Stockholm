<div class="box">
    <h1>Arkiv</h1>
    <p>
        Välkommen till det i nuläget minimala Arkiv som vi erbjuder.
        Tanken är den, att vi ska samla allt från texter till
        animeringar som har anknytning till Blåvitt, fotboll och även
        sport i allmänhet.
    </p>
    <p>
        Om du skulle finna texter om oss på andra icke Blåvita sidor som
        du ej ser på denna site, maila länken till oss. Vi välkomnar
        materiel av alla de slag, så skicka dina alster till <?php echo
        html::mailto('stockholm@anglarna.se') ?>!
    </p>
</div>
<div class="box">
    <h2>Länkarkiv</h2>
    <dl>
        <dt>Blåvitt</dt>
            <dd>
                <ul>
                    <li><a href="http://www.facebook.com/group.php?gid=5846573434">Änglarna Stockholm på Facebook</a></li>
                    <li><a href="http://www.ifkgoteborg.se/">IFK Göteborgs</a></li>
                    <li><a href="http://www.anglarna.se/">Supporterklubben Änglarnas</a></li>
                    <li><a href="http://www.svenskafans.com/fotboll/ifkgoteborg/">Svenska Fans — Alltid Blåvitt</a></li>
                    <li><a href="http://www.baraben.com">BaraBen</a></li>
                    <li><a href="http://web.comhem.se/~u96800871/index.htm">Kombarris GBG</a></li>
                    <li><a href="http://hem.passagen.se/mala6694/">Änglarna Hofors</a></li>
                    <li><a href="http://hem.bredband.net/b106302/valkommen.htm">Änglarna Trestad</a></li>
                    <li><a href="http://skane.anglarna.se">Änglarna Skåne</a></li>
                    <li><a href="http://www.anglarnanord.tk/">Änglarna Nord</a></li>
                    <li><a href="http://hem.passagen.se/bolton/elig.html">ELIG</a></li>
                    <li><a href="http://www.angelz.net">BrownBombers</a></li>
                </ul>
            </dd>
        <dt>Svensk Fotboll</dt>
            <dd>
                <ul>
                    <li><a href="http://www.fotbollsallsvenskan.se/">Fotbollsallsvenskan.se</a> (tabell, matcher, mm)</li>
                </ul>
            </dd>
    </dl>
</div>
<div class="box">
    <h2>Textarkiv</h2>
    <p>
        Textarkivet är tomt! De gamla artiklarna är på gång; under tiden
        får ni gärna tipsa oss om artiklar via <?php echo html::mailto('stockholm@anglarna.se') ?>!
    </p>
</div>